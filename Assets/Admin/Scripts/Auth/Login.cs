...
public class Login : MonoBehaviour
{
    ...
    private IEnumerator TryLoginCoroutine()
    {
        ...
        if (Int32.TryParse(_responseText, out int emailMatchedQuantity))
            ...
            {
                _authFieldsManipulator.MessageThrowUI(_errorAuth, "Пользоввателя с таким адресом электронной почты не существует", false);
                _canLogin = true;
                yield break;
            }
        ...
        if (!string.IsNullOrEmpty(CurrentUser.email))
        {
            if (CurrentUser.access_level == 0)
                _authFieldsManipulator.MessageThrowUI(_errorAuth, "Пользователь не активирован администратором музея. Вход запрещен", false);
            else
            {
    ...
    private bool IsLoginInfoWrong()
    {
        ...
        {
            _authFieldsManipulator.MessageThrowUI(_errorAuth, "Пароль не может быть меньше 8 или больше 24 символов", false);
            return true;
        }
        ...
        if (!match.Success)
        {
            _authFieldsManipulator.MessageThrowUI(_errorAuth, "Адрес электронной почты не соответствует правилам ввода", false);
            return true;
        }
