...
public class Registration : MonoBehaviour
{
    ...
    private IEnumerator RegistrationCoroutine()
    {
        ...
        if (Int32.TryParse(_responseText, out int emailMatchedQuantity))
            ...
            {
                _authFieldsManipulator.MessageThrowUI(_errorReg, "Пользователя с таким адресом электронной почты уже существует", false);
                _canRegister = true;
                yield break;
            }
        ...
        if (_responseText.Split(' ')[0] == "Registered")
            ...
            _authFieldsManipulator.MessageThrowUI(_errorReg, "При регистрации произошла непредвиденная ошибка", false);
    ...
    private bool IsRegistrationInfoWrong()
    {
        ...
        {
            _authFieldsManipulator.MessageThrowUI(_errorReg, "Пароль не может быть меньше 8 или больше 24 символов", false);
            return true;
        }
        ...
        if (!match.Success)
        {
            _authFieldsManipulator.MessageThrowUI(_errorReg, "Адрес электронной почты не соответствует правилам ввода", false);
            return true;
        }
